...
var VideoPlayer = $hx_exports["VideoPlayer"] = function(state) {
	...
	this.addCommand("toggle-fullscreen",function(e4) {
		_gthis.toggleFullscreen();
	});
	this.addCommand("toggle-controls",function(e5) {
		_gthis.video.controls = !_gthis.video.controls;
	});
	this.addCommand("seek-backward",function(e6) {
		if(_gthis.video != null) {
			_gthis.seek(-_gthis.calcSeekValue(e6.originalEvent != null && e6.originalEvent.shiftKey));
		}
	});
	this.addCommand("seek-forward",function(e7) {
		if(_gthis.video != null) {
			var tmp = _gthis.calcSeekValue(e7.originalEvent != null && e7.originalEvent.shiftKey);
			_gthis.seek(tmp);
		}
	});
	this.addCommand("toggle-mute",$bind(this,this.toggleMute));
	this.addCommand("volume-increase",function(e8) {
		if(_gthis.video != null) {
			_gthis.video.volume = Math.min(_gthis.video.volume + 0.1,1.0);
		}
	});
	this.addCommand("volume-decrease",function(e9) {
		if(_gthis.video != null) {
			_gthis.video.volume = Math.max(_gthis.video.volume - 0.1,0.0);
		}
	});
