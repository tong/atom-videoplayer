...
var VideoPlayer = function(state) {
	...
	this.commands.add(atom.commands.add(this.element,"videoplayer:" + "mute",function(e1) {
		_gthis.video.muted = !_gthis.video.muted;
		return;
	}));
	this.commands.add(atom.commands.add(this.element,"videoplayer:" + "seek-forward",function(e2) {
	...
	this.commands.add(atom.commands.add(this.element,"videoplayer:" + "seek-backward",function(e3) {
		...
		_gthis.video.currentTime += -v1;
		return;
	}));
	this.commands.add(atom.commands.add(this.element,"videoplayer:" + "volume-increase",function(e4) {
		return _gthis.video.volume = Math.min(_gthis.video.volume + 0.1,1.0);
	}));
	this.commands.add(atom.commands.add(this.element,"videoplayer:" + "volume-decrease",function(e5) {
		return _gthis.video.volume = Math.max(_gthis.video.volume - 0.1,0.0);
	}));
	this.commands.add(atom.commands.add(this.element,"videoplayer:" + "toggle-fullscreen",function(e6) {
		_gthis.toggleFullscreen();
		return;
	}));
	this.commands.add(atom.commands.add(this.element,"videoplayer:" + "goto-start",function(e7) {
		return _gthis.video.currentTime = 0;
	}));
	this.commands.add(atom.commands.add(this.element,"videoplayer:" + "screenshot",function(e8) {
		...
		js_node_Fs.writeFile(path,dataURI,{ encoding : "base64"},function(e9) {
			if(e9 != null) {
	...
	atom.config.onDidChange("videoplayer.playback.autoplay",{ },function(e10) {
		return _gthis.video.autoplay = e10.newValue;
	});
	atom.config.onDidChange("videoplayer.playback.loop",{ },function(e11) {
		return _gthis.video.loop = e11.newValue;
	});
	atom.config.onDidChange("videoplayer.scale",{ },function(e12) {
		_gthis.setScaleMode(e12.newValue);
		return;
	});
	atom.config.onDidChange("videoplayer.background",{ },function(e13) {
		var tmp = e13.newValue.transparent ? null : e13.newValue.color.toHexString();
	...
	if(state != null) {
		...
		if(state.play) {
			this.video.oncanplaythrough = function(e14) {
